document.addEventListener('DOMContentLoaded', () => {
    ...
    let cart = JSON.parse(localStorage.getItem('erpsmp_passes_cart')) || [];
    ...
    // UI Updates
    const saveCart = () => {
        localStorage.setItem('erpsmp_passes_cart', JSON.stringify(cart));
    };
    ...
    const updateCartUI = () => {
        ...
        cart.forEach(item => {
            ...
            itemEl.innerHTML = `
                <div class="cart-item-icon-block" style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); width: 60px; height: 60px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 24px; color: ${product.color}; flex-shrink: 0;">
                    <i class="fa-solid ${product.icon}"></i>
                </div>
                <div class="cart-item-info" style="margin-left: 14px;">
                    <h4>${product.name}</h4>
    ...
    if (btnProceedCheckout) {
        btnProceedCheckout.addEventListener('click', () => {
            alert(`🎉 Thank you, ${checkoutUsername}! Your game pass rank has been successfully linked to your ${checkoutPlatform.toUpperCase()} account.`);
            cart = [];
            updateCartUI();
            closeCheckoutModal();
        });
    ...
    // Add to Cart buttons for Passes
    document.querySelectorAll('.btn-add-pass-cart').forEach(btn => {
